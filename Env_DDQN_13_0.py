...
class CellularEnv(Env):
    def __init__(self, Nue, Nbs, Rmin, Rmax, cell_deployment, path_loss, A, intervals,
        ...
        # Validation set for testing 15,15 and 20 scenarios for Lvl1, Lv2 and Lvl 3 Respectively
        ...
        #self.C = self.Nbs - 1  # Interferers

        #self.state = np.zeros((self.Nbs, (self.Nbs * 2) + self.C), dtype=np.float32)  # {R_, P_, SINR_}
        ...
        #if len(self.cell_deployment) >4: # Poisson BS deploymen is implemented
        ...
        # For Validation ---------------------------------------------------------------------------------------------
        self.channel_gain_val = np.copy(self.channel_gain)
    ...
    def step(self, action):
        ...
        # Individual Reward
        #self.reward_rate = self.rewards[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs]
    ...
    def reset(self):
        ...
        if self.cell_deployment == 'poisson':  # Poisson BS deploymen is implemented
            ...
            #4: 78, 45, # 7: 78, 90 # 19: 180, 180
            # 4: 78, 45, # 7: 80, 80 # 19: 180, 180
    ...
    def validation1_reset(self, test_number): # There are 50 UE deployment positions fixed
        ...
        self.channel_gain = np.copy(self.channel_gain_val)
    ...
    def FP_power2(self):
        ...
        Rates1 = []
        for index in range(self.intervals):  # para cada intervalo
            P=np.random.uniform(0, self.Pmax, self.Nbs) # Random power initialitation
            for i_agent_BS in np.arange(self.Nbs):
                idy = self.active_users[i_agent_BS]
                idx = self.UE_BS_index[idy]
                # Formating for operations -------------------------------------------------------------------------
            ...
            # Gamma initialization (Step 0)
            #gamma = (H2[:, 0] * P_matrix[:, 0]) / (np.sum(H2[:, 1:] * P_matrix[:, 1:], axis=1) + self.N)
            #gamma = np.random.uniform(10 ** (-15 / 10), 1000, (self.Nbs))

            # Repeat until convergence or 100 iterations*
            fq_CF = 0
            for cou in range(100):
                fq_CF_last = np.copy(fq_CF)
                # Update y by (45) (Step 1)
                y = (np.sqrt(W * (1. + gamma) * H2[:, 0] * P_matrix[:, 0]) ) / np.sum ((H2[:, 1:] * P_matrix[:, 1:]) + self.N)
                # Update Gamma by (42) (Step 2)
                gamma = (H2[:, 0] * P_matrix[:, 0]) / (np.sum(H2[:, 1:] * P_matrix[:, 1:], axis=1) + self.N)
                # Update p by (44) (Step 3)
                Y = np.zeros((self.Nbs, self.Nbs))
                for i_agent_BS in np.arange(self.Nbs):
                    idy = self.active_users[i_agent_BS]
                    #P_matrix[i_agent_BS, :] = np.roll(P, -i_agent_BS)
                    Y[i_agent_BS,:] = np.roll(y, -i_agent_BS)

                P= np.minimum(self.Pmax, (np.square(Y[:,0])*W*(1+gamma)*H2[:,0]) / np.square(np.sum(np.square(Y[:,1:]) * H2[:,1:])))

                # Until function fq_CF in (41) converges
                for i_agent_BS in np.arange(self.Nbs): # New Pmatrix
                    idy = self.active_users[i_agent_BS]
                    P_matrix[i_agent_BS, :] = np.roll(P, -i_agent_BS)
                    #Y[i_agent_BS,:] = np.roll(y, -i_agent_BS)
                fq_CF = np.sum(W*np.log(1+gamma)) - np.sum(W * gamma) + np.sum((W*(1+gamma)*H2[:,0]*P_matrix[:,0]) /
                                                                               np.sum(H2[:,1:]*P_matrix[:,1:])+self.N)
                #print('fq_CF_last', fq_CF_last)
                #print('fq_CF', fq_CF)
                if np.linalg.norm(fq_CF_last - fq_CF) < 1e-3:
                #if (fq_CF_last - fq_CF) < 1e-3:
                    #print('-----------BREAK----------------------------')
                    #print('Iteraciones', cou)
                    #print('Potencias', P)
                    #wait = input('Press enter to continue')
                    break
                #wait= input('Press enter to continue')
