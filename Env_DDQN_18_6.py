...
'''
It seems that the shadowing is not calculated correctly and it has a high variation on the Path-loss
causing that the allocation has a little significant impact, cause the network performance is dominated by this
variation...
For the above, I decided to modified based on experiments on Matlab as follows:

        lognormal = 8 * np.random.randn(self.Nbs, self.Nue)
        variance = pow(10.,-(path_loss_LTE(distances) + lognormal) / 10.)
        ...
        channel_set[:, :, 0] = np.sqrt(
            variance/2) * (np.random.randn(self.Nbs, self.Nue) ** 2 + j*np.random.randn(self.Nbs,self.Nue) ** 2)
        for i in range(1, int(intervals)):
            channel_set[:,:,i] = channel_set[:, :, i-1] * pho + np.sqrt((1 - pho) * (variance / 2) *
            (np.random.randn(self.Nbs, self.Nue) ** 2 + j*np.random.randn(self.Nbs,self.Nue) ** 2))
...
def Generate_state(reward_modified, power_modified, sinr_norm_inv, Nbs, C):
    ...
    #print('Usuarios interferences: ', C)
    #print('Usuarios ', Nbs)
    if Nbs > C:
        rate_state = np.hstack([reward_modified[:, 0:1], reward_modified[indices1, indices2 + 1]])
        power_state = np.hstack([power_modified[:, 0:1], power_modified[indices1, indices2 + 1]])
...
class CellularEnv(Env):
    def __init__(self, Nue, Nbs, Rmin, Rmax, cell_deployment, path_loss, A, intervals,
                 Pmin_dBm, Pmax_dBm, noise_power_dBm, SINR_th, C, Validation_set, Validation):
        ...
        # Pathloss 1 LTE
        variance = pow(10., -(path_loss_LTE(distances) + lognormal) / 10.)
        # Pathloss 2 Dual pathloss
        # dual_slope_path_loss_matrix(d, K0, alfa1, alfa2, dBP, path_loss)
        #variance = pow(10., -(dual_slope_path_loss_matrix(distances, self.K0, self.alfa1, self.alfa2, self.dBP, self.path_loss) + lognormal) / 10.)


        channel_set = np.zeros([self.Nbs, self.Nue, self.intervals], dtype='complex_')
        pho = np.float32(scipy.special.k0(2 * np.pi * self.fd * self.Ts))
        channel_set[:, :, 0] = np.sqrt(
            variance / 2) * (np.random.randn(self.Nbs, self.Nue) ** 2 + 1j * np.random.randn(self.Nbs, self.Nue) ** 2)

        for i in range(1, int(intervals)):
            channel_set[:, :, i] = channel_set[:, :, i - 1] * pho + np.sqrt((1 - pho) * (variance / 2) *
                            (np.random.randn(self.Nbs, self.Nue) ** 2 + 1j * np.random.randn(
                                                                                self.Nbs, self.Nue) ** 2))
        self.channel_gain = np.square(abs(channel_set)) #************ CHECK****

        #print(self.channel_gain)

        #import matplotlib.pyplot as plt
        #plt.hist(10*np.log10(self.channel_gain.flatten()), bins=100)
        #plt.show()
        #wait = input('Press enter to continue')
            ...
            for test_number in np.arange(Val_scenarios):
                if self.cell_deployment == 'poisson':  # Poisson BS deploymen is implemented
                    ...
                    x_base_station, y_base_station, x_user, y_user = Simulation_Scenario(self.Nbs, self.Nue,
                                                                                                   10, 1, width, height)
                    #print(x_base_station)
                    #print(y_base_station)
                    #print(self.val_cell_position)
                    #print(np.vstack((x_base_station, y_base_station)).T)

                    #self.val_cell_position[:,:,test_number] = np.vstack((x_base_station, y_base_station)).T
                    self.val_cell_position = np.vstack((x_base_station, y_base_station)).T
                ...
                # Pathloss 2 Dual pathloss
                # dual_slope_path_loss_matrix(d, K0, alfa1, alfa2, dBP, path_loss)
                #variance = pow(10., -(dual_slope_path_loss_matrix(distances, self.K0, self.alfa1, self.alfa2, self.dBP,
                #                                                  self.path_loss) + lognormal) / 10.)

                channel_set = np.zeros([self.Nbs, self.Nue, self.intervals], dtype='complex_')
                pho = np.float32(scipy.special.k0(2 * np.pi * self.fd * self.Ts))
                channel_set[:, :, 0] = np.sqrt(
                    variance / 2) * (np.random.randn(self.Nbs, self.Nue) ** 2 + 1j * np.random.randn(self.Nbs,
                                                                                                     self.Nue) ** 2)

                for i in range(1, int(intervals)):
                    channel_set[:, :, i] = channel_set[:, :, i - 1] * pho + np.sqrt((1 - pho) * (variance / 2) *
                                                                                    (np.random.randn(self.Nbs,
                                                                                                     self.Nue) ** 2 + 1j * np.random.randn(
                                                                                        self.Nbs, self.Nue) ** 2))
                channel_gain = np.square(abs(channel_set))  # ************ CHECK****
                self.val_channel_gain[:, :, :, test_number] = channel_gain
    ...
    def Test(self):
        print(self.Nbs, ' Base Stations' )


    def step(self, action):
        # --------------------- Reward Calculation -------------------------------------------------------------------
        for i in np.arange(self.Nbs):  # Action is a vector of len(Nbs)
            #print('Numero de usuarios:', self.Nbs)
            #print('Tamaño de Tx_power', self.Tx_power.shape)
            #print('Vector de acciones:', action)
            #print('Vector de Tx_power', self.Tx_power)
        ...
        #print(self.channel_gain[:,:,self.episode_length])
        ...
        for i in np.arange(self.Nbs):
            ...
            Interference = np.sum(
                self.channel_gain[self.UE_BS_index[aux], i, self.episode_length] * self.Tx_power[aux])
            ...
            if sinr >= self.SINR_th:  # Minimum SINR Restriction
                ...
                '''
                if sinr >0:
                    R_value += np.exp((sinr / self.SINR_th) - 1)
                else:
                    R_value -= 1
                '''
        ...
        # Just The sum rate
        #self.reward_rate = np.ones(self.Nbs) * sumrate

        # KPI - 2
        #self.EE.append(
        #    np.divide(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs],
        #              self.Tx_power, where=self.Tx_power!=0))
        #EE = sum(
        #   np.divide(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs],
        #             self.Tx_power, where=self.Tx_power!=0))

        #EE_ind = np.divide(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs],
        #          (10 * np.log10(1000 * (self.Tx_power), where=self.Tx_power != 0)), where=self.Tx_power != 0)

        EE_ind = np.divide(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs],
                  self.Tx_power, where=self.Tx_power != 0)

        EE = sum(EE_ind)
        # Transformed to bps/dBW
        #EE = 10*np.log10(EE, where=EE != 0)

        # ----------------------------- REWARD FUNCTION (Z5) ---------------------------------------------------------
        #self.reward_rate = np.ones(self.Nbs) * (R_value/self.Nbs)

        aux_arr = np.array(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs])

        index1 = np.where(np.array(aux_arr) == 0)[0]
        index2 = np.where(np.array(aux_arr) != 0)[0]
        self.reward_rate[index1] = -1
        self.reward_rate[index2] = 1-np.exp(-1*aux_arr[index2])

        # Reward generalized to all users [-1, 1]
        self.reward_rate = np.ones(self.Nbs) * np.mean(self.reward_rate)
        # ----------------------------- REWARD FUNCTION (Z5) ---------------------------------------------------------

        # - - - - - - - Z6 --------
        '''
        self.reward_rate[index2] = 1 - np.exp(np.log10(1+self.SINR_th) - 1 * aux_arr[index2])
        print('Minimum rate: ',  np.exp(np.log10(1+self.SINR_th)))
        print('Rates: \n', aux_arr[index2])
        '''

        QoS = counter / self.Nbs # Percentaje of UEs
        ...
        # Reward Function: Throughput Restricted by user not served --------------------------------------------
        #counter = self.Nbs - np.count_nonzero(self.rates[(self.episode_length *
        #                                                self.Nbs):(self.episode_length * self.Nbs) + self.Nbs])
        #self.reward_rate = np.ones(self.Nbs) * (sumrate - (counter * 10))
        # Reward Function: Individual Reward Restricted by user not served -------------------------------------
        # [https://sci-hub.mksa.top/10.1109/wcnc.2014.6952410]
        aux_arr = np.array(self.rates[(self.episode_length * self.Nbs):(self.episode_length * self.Nbs) + self.Nbs])
        ...
        #self.R_ = np.copy(self.reward_rate)
        ...
        #print(self.reward_rate / self.Nbs)
        #print(sumrate / self.Nbs)
        #wait = input('Press enter to continue')

        return self.state, self.reward_rate, done, info, sumrate, EE, sum(self.Tx_power), QoS

    def UE_position(self,Rmin, Rmax):
        self.x_user_set, self.y_user_set = user_set(100000, Rmin, Rmax)

    def reset(self, Nbs):
        # --------------------------------------------- VERSION 16.1 -----------------------------------------
        self.Nue = int(Nbs)
        self.Nbs = int(np.copy(self.Nue))
        self.Tx_power = np.zeros(self.Nbs)
        # --------------------------------------------- VERSION 16.1 -----------------------------------------
    ...
    def random_power(self):
        ...
        return sum(Rates_episode2) / (self.intervals), Rates2, Tx_power2
    ...
    def WMMSE_power2(self):
        ...
        return sum(Rates_episode3) / self.intervals, Rates3, Tx_power3
    ...
    def FP_power(self):
        ...
        for index in range(self.intervals):  # para cada intervalo
            ...
            # End of FP algorithm
            Tx_power4 = P
            Tx_power4[Tx_power4 <= self.Pmin] = 0
            H2 = self.channel_gain[:, :, index]
        ...
        return sum(Rates_episode4) / (self.intervals), Rates4, Tx_power4
